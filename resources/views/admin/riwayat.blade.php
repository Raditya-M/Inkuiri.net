<x-app-layout>
  <section class="min-h-screen text-white py-12">
    <!-- Applied glassmorphism background with animated orbs -->
        ...
        <div class="space-y-4">
          @foreach($riwayat as $item)
            <div class="backdrop-blur-xl bg-white/10 border border-white/20 rounded-xl p-6 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-6 hover:border-white/40 hover:bg-white/15 transition-all duration-300 group">
              <!-- Glassmorphic card with improved layout and color scheme -->
              <div class="flex-1">
                <h2 class="font-bold text-xl text-white group-hover:text-blue-300 transition-colors">{{ $item->book->judul }}</h2>
                <div class="mt-3 space-y-2 text-sm">
                  <p class="text-slate-300">
                    Pembeli: <span class="font-medium text-blue-300">{{ $item->user->name }}</span>
                  </p>
                  <p class="text-slate-300">Tanggal Beli: <span class="font-medium text-blue-300">{{ $item->tanggal_pinjam->format('d M Y') }}</span></p>
              ...
              <div class="flex-shrink-0">
                <img src="{{ asset('storage/' . $item->book->gambar) }}" class="w-20 h-28 object-cover rounded-lg shadow-lg border border-white/20 hover:border-white/40 transition-all" alt="{{ $item->book->judul }}">
              </div>
            </div>
          @endforeach
        </div>
